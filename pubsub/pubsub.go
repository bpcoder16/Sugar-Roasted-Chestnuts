...
var redisPubSubManager *websocket.RedisPubSub
...
func RedisSubscribe(ctx context.Context, m *websocket.ClientManager) (err error) {
	panic(redisPubSubManager.Subscribe(ctx, cRedis.DefaultClient(), func(msg *redis.Message) {
		...
		if pubSubMessage.Scene == SceneTest && len(pubSubMessage.ID) > 0 {
			m.Range(sceneTest(ctx, pubSubMessage))
		}
	}))
}
